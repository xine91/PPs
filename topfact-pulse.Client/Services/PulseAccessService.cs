...
namespace topfact.Pulse.Services
{
    ...
    public class PulseAccessService : IPulseAccessService
    {
        ...
        {
            var accessRecords = await _context.PulseAccess
                .Where(x => x.Username == username && x.IsActive && x.BereichID == bereichId)
                .Where(x => x.GruppeID.HasValue)
                .Select(x => x.GruppeID.Value)
                .Distinct()
                .ToListAsync();

            // Hat der Benutzer Zugriff auf alle Gruppen im Bereich?
            var hasBereichAccess = await _context.PulseAccess
                .AnyAsync(x => x.Username == username && x.IsActive && x.BereichID == bereichId && x.GruppeID == null);

            if (hasBereichAccess)
            {
                // Rückgabe aller Gruppen im Bereich
